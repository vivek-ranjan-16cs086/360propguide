            ...
            <div class="col-sm-12">
                <div class="row align-items-center justify-content-between">
                    <div class="d-flex w-fit">
                        <h2 class="text-decoration-none text-dark customFont me-2 fw-medium h5 d-none d-md-block">Sort
                            By:</h2>
        ...
        <div class="row">
                ...
                <div id="desktopFilterContent">
                    ...
                    <div class="filter-box recommended h-fit" id="filter-form">
                        ...
                        <div class="location py-3 property-type location" id="resetLocation">
                            ...
                            @php
                            $isChecked = isset($filters['location']) && in_array($location, (array)
                            $filters['location']);
                            if (!$isChecked && isset($filters['city'])) {
                            $isChecked = strtolower($filters['city']) === strtolower($location);
                            }
                            @endphp
                            <input onchange="updateLocation(); handleCityChange()" type="checkbox"
                                class="property-filter" id="location-{{ $index }}" value="{{ $location }}"
                                name="location[{{ $index }}]" {{ $isChecked ? 'checked' : '' }}>
                        ...
                        <div class="py-3 property-type locality" id="resetLocality">
                            ...
                            @php
                            $isChecked = isset($filters['locality']) && in_array($localities, (array)
                            $filters['locality']);
                            @endphp

                            <input type="checkbox" onchange="updateLocality()"
                                class="property-filter locality-item {{ $index >= 5 ? 'd-none extra-location' : '' }}"
                                id="localities-{{ $index }}" value="{{ $localities }}" name="locality[{{ $index }}]"
                                data-city="{{ $localityCityMap[$localities] ?? '' }}" data-index="{{ $index }}"
                                {{ $isChecked ? 'checked' : '' }}>
                        ...
                        <div class="developer py-3 property-type developer" id="resetDeveloper">
                            ...
                            @php
                            $isChecked = isset($filters['developer']) && in_array($developer, (array)
                            $filters['developer']);
                            @endphp

                            <input onchange="updateDeveloper()" type="checkbox"
                                class="property-filter {{ $index >= 5 ? 'd-none extra-developer' : '' }}"
                                id="developer-{{ $index }}" value="{{ $developer->id }}" name="developer[{{ $index }}]"
                                {{ $isChecked ? 'checked' : '' }}>
            ...
            <div class="col-lg-9">
                ...
                <div class="row defaultSpace" id="project-list">
                    ...
                    <div class="col-sm-6 col-md-4 col-lg-4 pb-4">
                        <a href="{{ route('projects.details', $project) }}" style="text-decoration:none;">
                            <div class="pb-4 customborder position-relative projectCard border">
                                ...
                                <div>
                                    <img src="{{ url('storage/'.$project->logo_image) }}" class="w-100"
                                ...
                                <div class="ms-3 customFontColour">
                                    ...
                                    <div>
                                        <p class="m-0">{{ $project->typology }}</p>
                                    </div>
                                </div>
                    ...
                    @endforeach
                    @else
                    <div class="defaultSpace text-center">
                ...
                <div id="pagination-links">
                    <ul class="pagination">
                        ...
                        @endphp
                        <li class="page-item {{ $projects->onFirstPage() ? 'disabled' : '' }}"><a
                                class="page-link">Previous</a></li>

                        @if($lastPage <= 5) @for($page=1; $page <=$lastPage; $page++) <li
                            class="page-item {{ $currentPage === $page ? 'active' : '' }}"><a class="page-link"
                            ...
                            @for($page = $startPage; $page <= $endPage; $page++) <li
                                ...
                                @if($currentPage < $lastPage - 2) @if($currentPage < $lastPage - 3) <li
                                    class="page-item disabled"><span class="page-link">...</span></li>
                                    @endif
                                    <li class="page-item"><a class="page-link"
                                            data-page="{{ $lastPage }}">{{ $lastPage }}</a></li>
                                    @endif
                                    @endif

                                    <li class="page-item {{ $currentPage === $lastPage ? 'disabled' : '' }}"><a
...
$("#slider-range").slider({
    ...
    slide: function(event, ui) {
        $("#amount").html(
            "<div class='my-3'>" + formatPrice(ui.values[0]) +
            "</div> <div class='my-3'>-</div> <div class='m-3'>" + formatPrice(ui.values[1]) + "</div>"
        );
        filtersData['budget'] = {
            'min': ui.values[0],
            'max': ui.values[1]
...
$("#amount").html("<div class='my-3'>" + formatPrice(defaultMin) +
    "</div> <div class='my-3'>-</div><div class='my-3'>" + formatPrice(defaultMax) + "</div>");
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.status) {
                        ...
                        pagination +=
                            `<li class="page-item ${prevClass}"><a class="page-link">Previous</a></li>`;
                        ...
                        pagination +=
                            `<li class="page-item ${nextClass}"><a class="page-link">Next</a></li>`;
...
$('#resetFilters').on('click', function() {
    ...
    $("#amount").html("<div class='my-3'>" + formatPrice({
        {
            $minPrice
        }
    }) + "</div> <div class='my-3'>-</div><div class='my-3'>" + formatPrice({
        {
            $maxPrice
        }
    }) + "</div>");
